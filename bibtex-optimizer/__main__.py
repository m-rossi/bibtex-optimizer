    ...
    if match is not None:
        ...
        if os.path.isfile(filePath + '/' + match.group(0)) or \
            ...
            if entrytype == 'Standard':
                standardtype = re_field.search(re_type.search(entry).group(0))\
                    .group(0)
                number = re_field.search(re_number.search(entry).group(0))\
                    .group(0)
                revision = re_field.search(re_revision.search(entry).group(0))\
                    .group(0)
                ...
                if len(authors) > 0:
                    ...
                    for jj in range(len(authors)):
                        authors[jj] = re_surname.search(authors[jj]).group(0)
            ...
            filename = entrytype + '/' + filename[0: args.length] + extension
            # ask for file change
            if filename != oldpath:
                ...
                if userInput == 'y':
                    ...
                    if os.path.isfile(filePath + '/' + filename):
                        if args.overwriteall:
                            ...
                            userInput = 'u'
                        while userInput not in ['y', 'n']:
                            userInput = input('Found a file at destination, '
                                              'overwrite? [y/n]: ')
                    else:
                        userInput = 'y'
                    if userInput == 'y':
                        ...
                        if os.path.isfile(filePath + '/' + oldpath):
                            shutil.move(filePath + '/' + oldpath, filePath +
                                        '/' + filename)
                        else:
